...
def welcome_text(first_name: str, connected: bool = False) -> str:
    if connected:
        return (
            ...
            "❔ <b>Chatbot akkauntingizga ulangan</b> — sizga yozadigan "
            "odamlarga avto javob beradi.\n\n"
            "❓ Quyidagi tugmalar orqali buyruqlar, avto javoblar va "
            "sozlamalarni boshqaring 📱"
        )
    return (
        f"👋 Salom {first_name} | <b>{_now_time()}</b>\n\n"
        "❓ Bu botni akkauntingizga ulasangiz, sizga yozadigan odamlarga "
        "avto javob beradi va boshqa zo'r funksiyalar bor.\n\n"
        "❓ Qanday ulanadi:\n\n"
        "1. \"🔗 Botni ulash\" tugmasini bosing.\n\n"
        "2. Telegram Business ➝ Chatbots bo'limiga kirib, qidiruvga: "
...
def profile_text(user) -> str:
    ...
    if user.tariff == "pro":
        ...
        tariff_line = f"🆓 Start tarif (bepul)"
    lines = [
        "👤 <b>Profil</b>\n",
        tariff_line,
        "",
...
def tariff_compare_text() -> str:
    ...
    return (
        ...
        "  • 🟢 24/7 online ❌\n"
        "  • 🕵️ Shpion rejimi ❌\n\n"
        "💎 <b>Pro</b> — <b>{price:,} so'm/oy</b>\n".replace(",", " ").format(price=pro['price_month']) +
...
def invoice_text(amount: int) -> str:
    return (
        ...
        "<i>(Demo rejim: bu yerda haqiqiy to'lov amalga oshmaydi, chunki "
        "siz Bepul tarifdasiz va balansingiz mavjud emas.)</i>"
    )
...
def demo_payment_pending_text() -> str:
    return (
        ...
        "Bu — demo rejim, shuning uchun haqiqiy to'lov amalga oshirilmaydi. "
        "Ishlab chiquvchi to'lov tizimini (Payme/Click/Stars) ulaganidan so'ng "
        "bu joyda avtomatik tasdiqlash ishlaydi."
    )
...
def connect_bot_text() -> str:
    return (
        "❓ Bu botni akkauntingizga ulasangiz, sizga yozadigan odamlarga "
        "HAQIQIY avto javob beradi (Telegram Business orqali).\n\n"
        "❓ Qanday ulanadi:\n\n"
        "1. Pastdagi \"⚙️ Sozlamalarni ochish\" tugmasini bosing (yoki "
        "qo'lda Telegram ➝ <b>Sozlamalar</b>ga o'ting) ➝ <b>Telegram "
        "Business</b> ➝ <b>Chatbots</b> bo'limiga kiring.\n\n"
        f"2. \"Qo'shish\" tugmasini bosing va qidiruvga <b>@{BOT_USERNAME}</b> "
        "deb yozib, botni tanlang va ulaning.\n\n"
        "3. Tayyor! Ulanish tasdiqlangach, sizga alohida xabar keladi va "
        "shu paytdan boshlab sizga yozgan odamlarga bot avtomatik javob beradi.\n\n"
        "<i>Eslatma: bu funksiya faqat Telegram Business (Premium) obunasi "
        "bo'lgan akkauntlarda mavjud — bu Telegram'ning o'zi qo'ygan shart, "
        "botga bog'liq emas.</i>"
    )
...
def chatbot_settings_text(user) -> str:
    ...
    lines = [
        ...
        f"  <i>{mode_desc}</i>\n",
        f"⏰ Ish vaqti: <b>{'yoqilgan' if s.work_hours_enabled else 'o' + chr(39) + 'chirilgan'}</b>\n"
        f"  <i>{wh_desc}</i>\n",
    ...
    if not user.telethon_session:
        ...
        if wait > 0:
            ...
            lines.append(
                ...
                f"tez-tez ism o'zgartirilganda ishga tushadi). Taxminan "
...
def admin_home_text(pending: int = 0, banned: int = 0) -> str:
    ...
    return (
        "🛠 <b>Admin panel</b>\n"
        f"{alert_block}\n"
        "Quyidagi bo'limlardan birini tanlang, yoki tezkor buyruqlardan "
        "foydalaning:\n"
        "• <code>/credit user_id summa</code> — balansga qo'lda pul qo'shish\n"
        "• <code>/admin_stats</code> — qisqa statistika\n"
        "• 👥 <b>Foydalanuvchilar</b> bo'limida <code>user_id</code> yuborib "
        "istalgan foydalanuvchini boshqarishingiz mumkin (balans, blok, "
        "avto-javob xotirasini tozalash)."
    )


def admin_stats_text(s: dict) -> str:
    total = s['total'] or 1  # 0 ga bo'linishning oldini olish
    ...
    lines = [
        ...
        "",
        "💰 <b>Moliya</b>",
        f"  • Balanslar jami: <b>{s['balance_sum']:,} so'm</b>".replace(",", " "),
        f"  • O'rtacha balans: <b>{avg_balance:,} so'm</b>".replace(",", " "),
...
def admin_user_card_text(user) -> str:
    ...
    lines = [
        ...
        f"🏷 Tarif: {tariff_str}",
        f"🔗 Ulangan: {'✅ ha' if user.connected else '❌ bogʻlanmagan'}",
...
#
# Yuqoridagi barcha funksiyalar va HELP_TEXT/SPY_CONSENT_TEXT kabi tayyor
# matn konstantalari shu yerdan pastda AVTOMATIK ravishda pemoji.wrap()
# orqali "o'raladi": agar admin panelda biror emoji uchun premium ID
# o'rnatilgan bo'lsa, o'sha emoji matnda uchraganda premium ko'rinishda
# chiqadi. Bu FAQAT matnlarga tegishli — Telegram tugmalarida (keyboards.py)
# custom/premium emoji ishlatib bo'lmaydi, chunki buni Telegram Bot API
# platforma darajasida qo'llab-quvvatlamaydi.
# ============================================================================
